...
function Components() {
  ...
  const placeholders = ["Hero", "Navbar", "Footer", "Testimonial", "Pricing"];
  const tags = [
    "Hero",
    "Navbar",
    "Footer",
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filteredComponents = components.filter((component) =>
      component.tag.toLowerCase().includes(tag),
    );
  ...
  const handleTagClick = (tag: string) => {
    setActiveTag(tag);
    const filteredComponents = components.filter((component) =>
      component.tag.toLowerCase().includes(tag.toLowerCase()),
    );
    setFilteredComponents(filteredComponents);
  };
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {tags.map((tag) => (
                <button
                  ...
                  key={tag}
                  className={`border border-customDark px-2 py-1 rounded-md ${
                    activeTag === tag ? "bg-black" : "bg-customDark"
                  }`}
                >
